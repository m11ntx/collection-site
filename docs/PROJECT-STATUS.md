...
_Last updated: CS-65 — home gained a "Novidades" carousel (10 most recent
jerseys, sorted by `addedAt`, taken from `products.json`), placed right
before "Destaques Brasil"/"Destaques em geral". Unlike those two (curated by
hand in `featured.json` via `/admin`), this one is fully automatic and
self-refreshing on every home load — no admin/pipeline step needed, since
`addedAt` is already stamped by the pipeline on first import. See
`CHANGELOG.md` CS-65 for the implementation. Earlier, CS-64 — two mobile/navigation changes: (1) Brasileirão's
