...
(function () {
  ...
  function persist() { try { localStorage.setItem(KEY, JSON.stringify(items)); } catch (_) {} render(); }
  ...
  function mount() {
    ...
    root.innerHTML =
      `<button id="m11-fab" aria-label="Meu pedido"><span class="m11-bag">🛍️</span><span id="m11-count">0</span></button>
       <div id="m11-back"></div>
       <aside id="m11-drawer" role="dialog" aria-modal="true" aria-label="Meu pedido">
         ...
         <form id="m11-form" class="m11-hidden" novalidate>
           <p class="m11-form-title">Seus dados para o pedido</p>
           <label>Nome completo<input name="name" required autocomplete="name"></label>
           <label>WhatsApp<input name="phone" required inputmode="tel" autocomplete="tel" placeholder="(16) 99999-9999"></label>
           <label>E-mail <span class="m11-opt">(opcional)</span><input name="email" type="email" autocomplete="email" placeholder="voce@email.com"></label>
           <div class="m11-row">
             <label>Cidade <span class="m11-opt">(opcional)</span><input name="city" autocomplete="address-level2"></label>
             <label class="m11-uf">UF<input name="uf" maxlength="2" autocomplete="address-level1" placeholder="SP"></label>
           </div>
           <label>Observação <span class="m11-opt">(personalização, nome/nº…)</span><textarea name="note" rows="2"></textarea></label>
    ...
    document.getElementById("m11-items").addEventListener("click", onItemsClick);
    document.getElementById("m11-form").addEventListener("submit", onSubmit);
  ...
  function render() {
    ...
    document.getElementById("m11-items").innerHTML = items.length ? items.map((it, i) =>
      `<div class="m11-item">
         ...
         <div class="m11-info"><div class="m11-name">${esc(it.name)}</div>${it.size ? `<div class="m11-sz">Tam: ${esc(it.size)}</div>` : ""}<div class="m11-price">${money(it.price)}</div></div>
       ...
       </div>`).join("") : `<div class="m11-empty">Seu pedido está vazio.<br>Adicione camisas pelo botão “Adicionar ao pedido”.</div>`;
  ...
  function onItemsClick(e) {
    ...
    else if (rm) { items.splice(+rm.dataset.rm, 1); persist(); }
  }
  ...
  async function onSubmit(e) {
    ...
    const customer = {
      ...
      city: f.city.value.trim(), uf: f.uf.value.trim().toUpperCase(), note: f.note.value.trim(),
    };
    if (!customer.name || !customer.phone) {
      (customer.name ? f.phone : f.name).focus();
      const msg0 = document.getElementById("m11-msg"); msg0.textContent = "Preencha nome e WhatsApp."; msg0.className = "m11-err";
      return;
    }
    ...
    items = []; persist(); f.reset(); send.disabled = false;
  ...
  function waLink(customer) {
    const lines = items.map((i) => `• ${i.qty}x ${i.name}${i.size ? ` (${i.size})` : ""} — ${money(i.price)}`).join("\n");
    const dados = [
      `Nome: ${customer.name}`,
      customer.email ? `E-mail: ${customer.email}` : "",
      [customer.city, customer.uf].filter(Boolean).length ? `Cidade: ${[customer.city, customer.uf].filter(Boolean).join("/")}` : "",
  ...
  const CSS = `
    #m11-fab{ position:fixed; right:18px; bottom:18px; z-index:60; display:none; align-items:center; gap:.5rem;
      background:linear-gradient(180deg,#d4af5f,#b8924a); color:#17130c; border:0; border-radius:999px;
      padding:.7rem 1.05rem; font:600 .95rem system-ui,sans-serif; cursor:pointer; box-shadow:0 8px 24px rgba(0,0,0,.4); }
    #m11-fab .m11-bag{ font-size:1.1rem; }
    #m11-count{ display:grid; place-items:center; min-width:20px; height:20px; padding:0 5px; border-radius:999px; background:#17130c; color:#d4af5f; font-size:.75rem; font-weight:800; }
    #m11-back{ position:fixed; inset:0; z-index:61; background:rgba(0,0,0,.55); opacity:0; visibility:hidden; transition:opacity .2s; }
    #m11-drawer{ position:fixed; top:0; right:0; bottom:0; z-index:62; width:min(400px,92vw); background:#141417; color:#ededf1;
    ...
    .m11-item{ display:grid; grid-template-columns:52px 1fr auto auto; gap:.6rem; align-items:center; padding:.6rem 0; border-bottom:1px solid #23232a; }
    ...
    .m11-sz{ font-size:.72rem; color:#9a9aa4; } .m11-price{ font-size:.8rem; color:#d4af5f; font-weight:700; margin-top:.15rem; }
    ...
    .m11-form-title{ margin:0; font-weight:700; font-size:.95rem; color:#ededf1; }
    #m11-form label{ display:grid; gap:.3rem; font-size:.78rem; color:#9a9aa4; }
    .m11-opt{ color:#6f6f78; font-weight:400; }
    .m11-row{ display:grid; grid-template-columns:1fr 72px; gap:.7rem; }
