...
const Catalog = (() => {
    ...
    async function initReviewsPage() {
        ...
        const render = () => {
            root.innerHTML = `
                ...
                <form class="reviews-page__form" id="revForm" novalidate>
                    ...
                    <textarea class="reviews__field" name="comment" rows="4" maxlength="320" placeholder="${esc(I18N.t("reviews.comment"))}"></textarea>
                    <p class="reviews__counter" id="revCounter" aria-live="polite"></p>
        ...
        function wire() {
            ...
            // Contador de caracteres do comentário (digitado / total + faltam).
            const ta = form.comment, counter = root.querySelector("#revCounter");
            if (ta && counter) {
                const MAXC = ta.maxLength > 0 ? ta.maxLength : 320;
                const updc = () => {
                    const n = ta.value.length, rem = Math.max(0, MAXC - n);
                    counter.textContent = `${n}/${MAXC} · ${I18N.t("reviews.charsLeft", { n: rem })}`;
                    counter.classList.toggle("is-max", n >= MAXC);
                };
                ta.addEventListener("input", updc); updc();
            }
